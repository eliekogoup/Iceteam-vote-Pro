...
export async function checkEditionStatus(editionId: number) {
  ...
  try {
    ...
    // note: when using a join like `members!inner(...)`, Supabase may return `members` as an array
    // so we guard both shapes (object or array)
    const activeMembers = (groupMembers || []).filter((mg: any) => {
      const m = mg.members;
      if (!m) return false;
      // If it's an array, check the first element
      if (Array.isArray(m)) return m[0]?.is_active !== false;
